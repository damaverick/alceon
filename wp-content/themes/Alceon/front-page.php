...
?>
    ...
    ?>
      ...
      <p class="mt-5"><?php echo esc_html( get_field('stats_subtext') ); ?></p>
  <?php endif; ?>
  </div>
</section>
<section class="image-section image-section--full" aria-hidden="true">
  <?php 
  $fw_image = get_field('full_width_image');
  if( $fw_image ):
    // Get the image URL
    $fw_image_url = esc_url($fw_image['url']);
  ?>
    <div class="image-section__background" style="background-image: url('<?php echo $fw_image_url; ?>');"></div>
  <?php else: ?>
