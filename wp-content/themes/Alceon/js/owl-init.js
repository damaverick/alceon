jQuery(document).ready(function($) {
    ...
    if ($carousel.length > 0) {
        ...
        const prevIcon = '<img src="' + owlParams.themeUrl + '/img/left-arrow.svg" alt="Left">';
        const nextIcon = '<img src="' + owlParams.themeUrl + '/img/right-arrow.svg" alt="Right">';
    ...
    function wrapOwlControls(event) {
        ...
        // Logic to remove existing controls wrapper to prevent duplication
        if (carousel.parent().find('.bottom_slider').length > 0) {
            nav.add(dots).unwrap();
            carousel.parent().find('.bottom_slider').remove();
        }
        ...
        // Re-run the controls check logic inside the event
        // Note: we need to recalculate totalItems here or ensure the variable scope is accessible
        const currentItems = carousel.find('.owl-item').not('.cloned').length; 
        const showControls = (window.innerWidth < 1080) || currentItems > 4;
        
        carousel.parent().find('.bottom_slider').toggle(showControls);
    }
});